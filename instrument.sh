...
for p in 0.001 0.003 0.01; do
  for s in 1 2 3 4 5; do
    opt -enable-new-pm=0 -f -load ./build/libInsertSleepPass.so -insert-sleep -insert-sleep-seed $s -insert-sleep-probability $p -insert-sleep-time 3000 -S -o "${INPUT_FILE%.*}/sleep_${p}_${s}.ll" "${INPUT_FILE%.*}/vanila.ll"
